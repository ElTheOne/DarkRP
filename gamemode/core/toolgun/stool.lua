...
-- A dedicated server can register a gamemode-bundled SWEP before file.Find
-- exposes its virtual weapons/ directory. Keep the stock suite deterministic,
...
local stoolFiles = {
	...
	"material.lua", "motor.lua", "muscle.lua", "nocollide.lua", "paint.lua",
	"physprop.lua", "pulley.lua", "remover.lua", "rope.lua", "slider.lua",
	"precision.lua", "stacker_improved.lua", "thruster.lua", "trails.lua", "weld.lua",
	"wheel.lua", "winch.lua"
}
local knownStools = {}
local bundledStools = {}
for _, val in ipairs(stoolFiles) do
	knownStools[string.lower(val)] = true
	bundledStools[string.lower(val)] = true
end
for _, val in ipairs(file.Find(SWEP.Folder .. "/stools/*.lua", "LUA")) do
	local normalized = string.lower(val)
	if not string.StartWith(normalized, "._") and normalized ~= "example.lua" and not knownStools[normalized] then
		knownStools[normalized] = true
		stoolFiles[#stoolFiles + 1] = val
	end
end
...
for _, val in ipairs( stoolFiles ) do
	...
	local normalized = string.lower(val)
	local toolPath = bundledStools[normalized]
		and ("stools/" .. val)
		or (SWEP.Folder .. "/stools/" .. val)
	AddCSLuaFile( toolPath )
	include( toolPath )
