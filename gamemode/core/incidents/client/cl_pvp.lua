...
hook.Add("HUDPaint", "DRP.PVP.Notices", function()
	...
	for index, notice in ipairs(notices) do
		...
		local accent = notice.state == 0 and colors.green or (notice.state == 2 and Color(255, 190, 75) or colors.red)
		local titles = {
			[0] = "PVP ENDED WITH ",
		...
		local title = (titles[notice.state] or "PVP WITH ") .. string.upper(notice.other)
		local detail = notice.reason
		if notice.state == 1 then detail = detail .. "  •  One-way until you escalate" end
		if notice.state == 2 then detail = detail .. "  •  Return fire currently locked" end
		if notice.state ~= 0 and notice.grace > 0 then detail = detail .. "  •  " .. notice.grace .. "s grace" end
		...
		draw.SimpleText(title, "DRP.PVP.Title", x + 18, y + 18, Color(255, 255, 255, alpha), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText(detail, "DRP.PVP.Body", x + 18, y + 39, Color(colors.muted.r, colors.muted.g, colors.muted.b, alpha), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
end)
